...
```bash
curl -fsSL https://raw.githubusercontent.com/mikhaleff8230/treabo-ru-api/main/deploy/server/install-docker-ubuntu.sh -o /tmp/install-docker-ubuntu.sh
...
git clone git@github.com:mikhaleff8230/treabo-ru-api.git pixer-api
git clone git@github.com:mikhaleff8230/treabo-ru-shop.git shop
git clone git@github.com:mikhaleff8230/treabo-ru-admin.git admin
```
...
```bash
cd /var/www/treabo/pixer-api
export TREABO_DOMAIN=treabo.ru
export TREABO_ADMIN_DOMAIN=seller.treabo.ru
export TREABO_API_DOMAIN=api.treabo.ru
...
## 5.1 Admin (seller.treabo.ru)

Admin UI is on **seller.treabo.ru** (nginx → admin:3002). **Login API must be api.treabo.ru** — Marvel routes like `POST /token` are on the API host, not on seller.
...
```env
NEXT_PUBLIC_REST_API_ENDPOINT=https://api.treabo.ru
NEXT_PUBLIC_API_URL=https://api.treabo.ru
```
...
Create a Marvel super-admin for **Next.js admin** (`seller.treabo.ru/login`):

```bash
docker compose -p treabo -f deploy/treabo-compose.yml exec api php artisan treabo:create-admin \
  --email=admin@treabo.ru --password='YourSecurePass123' --name='Admin'
```
...
Do **not** open `https://api.treabo.ru/token` in the browser — that URL accepts **POST only** (login XHR from seller). A GET shows «405 Method Not Allowed» and is normal.

Filament at `api.treabo.ru/admin` is a separate Laravel UI and is **not** used by seller.treabo.ru.
...
- `https://seller.treabo.ru/login` — UI
- `https://api.treabo.ru/api/proffi-health` — API
- Browser devtools → login → request must go to `https://api.treabo.ru/token` (not seller.treabo.ru/token)
